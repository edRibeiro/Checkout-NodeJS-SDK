...
class CapturesRefundRequest {
  ...
  payPalAuthAssertion(clientId, payerId, email = null) {
    var auth_1 = btoa('{"alg":"none"}');
    var auth_2 = null;
    if (email) {
      var auth_2 = btoa('{"email":' + email + ',"iss":' + clientId + "}");
    } else {
      var auth_2 = btoa('{"payer_id":' + payerId + ',"iss":' + clientId + "}");
    }

    this.headers["PayPal-Auth-Assertion"] = auth_1 + "." + auth_2 + ".";
